...
export default function SeeGeneralEvent() {
    ...
    const handleDelete = (id) => {
        axios.delete(`http://localhost:8080/theater/deleteEvent/${id}`,`https://bkticketing-backend-production.up.railway.app/theater/deleteEvent/${id}`)
            .then(() => {
        ...
        {events.length > 0 ? (
            ...
            <tbody>
                ...
                <tr key={event.eventId}>
                    ...
                    <td>{event.theaterDate}</td>
                    <td>{event.theaterTime1}</td>
                    <td>{event.theaterVenue}</td>
                    <td>{event.theaterOrganizer}</td>
